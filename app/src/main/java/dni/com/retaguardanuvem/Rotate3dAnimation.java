...
public class Rotate3dAnimation extends Animation {
    ...
    public void initialize(int width, int height, int parentWidth, int parentHeight) {
        ...
        camera = new Camera();
        pivotY = resolveSize(mPivotXType, mPivotXValue, width, parentWidth);
        pivotY = resolveSize(mPivotYType, mPivotYValue, height, parentHeight);
    }

    @Override
    protected void applyTransformation(float interpolatedTime, Transformation t) {
        final  float fromDegress = this.fromDegrees;
        float degress = fromDegress + ((toDegrees - fromDegress) * interpolatedTime);
