    ...
    static Animation loadAnimation(Context context, int id)
        ...
        try(XmlResourceParser parser = context.getResources().getAnimation(id)){
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                throw new Resources.NotFoundException("Não foi possivel carregar animação recurso ID #0x" +
                ...
                default:
                  try {
                    anim = (Animation) Class.forName(name).getConstructor(Context.class, AttributeSet.class).newInstance(c, attrs);
                     ...
                     throw  new RuntimeException("Nome desconhecido da animação: " + parser.getName() + " error:" + te.getMessage());
                  }
                break;
            }
